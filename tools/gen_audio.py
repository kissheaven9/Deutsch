...
async def main():
    ...
    VERBS=['wohnen','leben','lernen','machen','arbeiten','studieren','planen','glauben',
           ...
           'aufmachen','vorbereiten','einstellen','schneiden','anfangen','zusammenarbeiten','leiten','helfen','einladen','gefallen','verkaufen','erzählen','bekommen','bezahlen','verdienen','planen','suchen','aufräumen','zählen','aufstehen','verpassen','vergessen','anschauen','verlieren','verbessern','verwalten','beraten','transportieren','organisieren','gründen','erfinden','ausprobieren','funktionieren','anrufen','kennen','aussehen','helfen','denken','mitarbeiten','einkaufen','abholen','besuchen','kennenlernen','zurückrufen']
    for inf in VERBS:
    ...
    for w in T20W:
        await tts(w, 'female', os.path.join(AUDIO,'word', slug(w)+'.mp3'))
    T20=[
      ('thema20-1','male','Otto hat eine tolle Idee: Er erfindet eine Garten-App! Die App ist für Menschen mit Garten. Viele Leute vergessen, ihre Blumen zu gießen. Die App erinnert sie jeden Tag. So vergisst niemand mehr seine Pflanzen! Otto steht jeden Morgen früh auf. Er kauft im Supermarkt ein, holt seinen Computer ab und arbeitet. Am Anfang hat die App nicht gut ausgesehen, aber Otto hat sie immer wieder verbessert. Greta hat die App zuerst ausprobiert. Sie hat Otto besucht, und die App hat ihr sehr gefallen! Greta hat allen davon erzählt — ihren Freunden und Kollegen. So hat Otto die ersten zehn Nutzer bekommen! Otto organisiert jetzt alles selbst. Er berät die Nutzer, und Greta arbeitet bei ihm mit. Wenn ein Nutzer anruft, ruft Otto schnell zurück. Manchmal transportiert er auch Pflanzen zu den Leuten. Ein Tag war chaotisch: Otto ist zu spät aufgestanden, hat den Bus verpasst und hat sein Handy verloren! Aber dann hat er neue Nutzer kennengelernt und sie in den Garten eingeladen. Was für ein Tag, hat Otto erzählt — müde, aber glücklich.'),
