...
# Первый референс — ЭТАЛОН СТИЛЯ серии (иллюстрация к тексту Темы 2)
STYLE_REF = 'dialog2-1b.png'
STYLE = ("Сделай иллюстрацию в ТОЧНО ТАКОМ ЖЕ художественном стиле, как первая приложенная "
         "картинка-эталон: богатая, детализированная ЖИВОПИСНАЯ книжная иллюстрация, тёплые "
         "насыщенные цвета, уютная атмосфера, множество декоративных деталей, мягкая живописная "
         "фактура (как в классической детской книге). НЕ плоско, не простая акварель, не мультяшно, "
         "не фотореализм. Персонажи должны выглядеть как на референсах-персонажах. "
         "Горизонтальная композиция. КАТЕГОРИЧЕСКИ без текста, букв, цифр, вывесок и надписей — "
         "все таблички и экраны пустые.")
...
def gen(name):
    ...
    refs = [STYLE_REF] + [r for r in refs if r != STYLE_REF]   # эталон стиля первым
    boundary = '----deutschimg'
    parts = []
    def field(n, v):
