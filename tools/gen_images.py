...
def style_ref(name):
    return 'otto-szene-1.png' if name.startswith('otto') else 'greta-szene-1.png'
STYLE = ("Сделай иллюстрацию в ТОЧНО ТАКОМ ЖЕ художественном стиле, как первая приложенная "
         "картинка-эталон: тёплая ПОЛУРЕАЛИСТИЧНАЯ МУЛЬТИПЛИКАЦИОННАЯ иллюстрация (2D cartoon "
         "illustration, как в качественном мультфильме/детской книге): округлые выразительные лица, "
         "мягкая объёмная светотень, тёплая насыщенная палитра, аккуратная детализация. "
         "НЕ фотореализм, НЕ плоская акварель. Персонажи ДОЛЖНЫ выглядеть КАК НА РЕФЕРЕНСАХ. "
         "Горизонтальная композиция. КАТЕГОРИЧЕСКИ без какого-либо текста, букв, цифр, вывесок, "
         "табличек и надписей — все вывески/экраны/таблички ПУСТЫЕ.")
